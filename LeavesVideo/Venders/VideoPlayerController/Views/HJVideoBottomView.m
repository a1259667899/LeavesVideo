...
static const CGFloat kTimeLabelWidth = 55.f;
...
- (void)refreshUI{
    ...
    if (self.configModel && self.configModel.onlyFullScreen && !self.onlySlider){
        ...
        [self.fullScreenBtn setHidden:true];
...
- (HJBufferSlider *)bufferSlider
{
    if (!_bufferSlider) {
        ...
        [_bufferSlider setProgressTrackColor:[UIColor colorWithRed:4.0/255 green:190.0/255 blue:254.0/255 alpha:1]];
        [_bufferSlider setBufferTrackColor:[UIColor whiteColor]];
        [_bufferSlider setMaxBufferTrackColor:[UIColor darkGrayColor]];
//        [_bufferSlider setThumbImage:[[UIImage imageNamed:@"play_slider_thumbImage"] imageWithSize:CGSizeMake(9, 9)]];
        [_bufferSlider setThumbImage:[UIImage imageNamed:@"play_slider_thumbImage"]];
...
- (void)show{
    UIDeviceOrientation origination = [[UIDevice currentDevice] orientation];
    ...
    CGFloat originX = 0;
    if (origination == UIDeviceOrientationPortrait) {
        originX = 0;
        width = CGRectGetWidth(self.superview.frame);
    }else if(origination == UIDeviceOrientationLandscapeLeft || origination == UIDeviceOrientationLandscapeRight){
        if (is_landSpaceX) {
            originX = 44;
            width = CGRectGetWidth(self.superview.frame) - 78;
        }
    }
...
- (void)hide{
    ...
    [UIView animateWithDuration:kDefaultAnimationDuration animations:^{
        self.frame = CGRectMake(originX, originY, width, kSliderHeight);
