...
- (void)changeFullScreen:(BOOL)changeFull{
    ...
    [UIView animateWithDuration:kDefaultAnimationDuration animations:^{
        if (changeFull) {
            ...
            if (is_landSpaceX) {//看是不是刘海屏
                UIDeviceOrientation orientation = [[UIDevice currentDevice] orientation];
                if (orientation == UIDeviceOrientationLandscapeRight) {
                    self.playerView.frame = CGRectMake(44, 0, kVideoFullFrame.size.width - 44 - 34, kVideoFullFrame.size.height - 34);
                }else if (orientation == UIDeviceOrientationLandscapeLeft){
                    self.playerView.frame = CGRectMake(34, 0, kVideoFullFrame.size.width - 44 -34, kVideoFullFrame.size.height - 34);
                }
            }else{//不是刘海屏
...
- (HJVideoBottomView *)bottomView{
    if (!_bottomView) {
        ...
        _bottomView.configModel = self.configModel;
...
//    let orientation = UIDevice.current.orientation
    if (self.configModel.onlyFullScreen == false) {
        UIDeviceOrientation orientation = [[UIDevice currentDevice] orientation];
