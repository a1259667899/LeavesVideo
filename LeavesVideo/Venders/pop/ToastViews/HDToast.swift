...
//        let msgstring = msg! as NSString
        let view = inView ?? self.window!
        let width = view.hd_width - 2 * marginH - 2 * insideMaginH - 30
        let msgrect = CGSize(width:width,height:CGFloat(MAXFLOAT))
        let sizeMsg = (msg! as NSString).boundingRect(with: msgrect, options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:UIFont.systemFont(ofSize: textFont)], context: nil).size
        ...
        let view1 = inView ?? self.window!
        self.imageToastView?.contentLabel.preferredMaxLayoutWidth = view1.hd_width-2*marginH-2*insideMaginH-30
...
extension HDToast {
    func show(msg:String?, inView:UIView? = nil, duration:Int?) {
        ...
        let sizeMsg = (msg! as NSString).boundingRect(with: CGSize(width:((inView ?? self.window!).hd_width)-2*self.marginH-2*insideMaginH,height:CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:UIFont.systemFont(ofSize: textFont)], context: nil).size
