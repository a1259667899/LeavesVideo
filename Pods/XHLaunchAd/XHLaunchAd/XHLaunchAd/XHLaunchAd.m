...
-(void)removeAndAnimate{
    /**适应扯淡的需求*/
    //    if (!_skipTimer){
    //        [self.skipButton setTitleWithSkipType:SkipTypeRoundText duration:10];
    //        [self.skipButton addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    //        return;
    //    }
    if (self.imageAdConfiguration.autoHide == false) {
        if (!_skipTimer){
            //修改按钮，不消失
            [self.skipButton setTitleWithSkipType:SkipTypeRoundText duration:10];
            self.imageAdConfiguration.autoHide = true;
            //        [self.skipButton addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
        }
        return;
    }
